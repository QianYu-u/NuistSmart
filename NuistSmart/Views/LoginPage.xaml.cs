...
namespace NuistSmart.Views
{
    ...
    public sealed partial class LoginPage : Page
    {
        ...
        private void CheckAndInterceptUrl(string url, CoreWebView2 webView, CoreWebView2NavigationStartingEventArgs? startArgs)
        {
            ...
            // 埋点日志：打印经过的所有 URL
            System.Diagnostics.Debug.WriteLine($"https://www.amazon.com/Monitors-Computers-Accessories/b?ie=UTF8&node=1292115011: {url}");
                ...
                url.Contains("i.nuist.edu.cn/default"))
            {
                ...
                // 立刻注销这两个事件的监听，防止重定向循环导致重复触发
                webView.NavigationStarting -= CoreWebView2_NavigationStarting;
                webView.SourceChanged -= CoreWebView2_SourceChanged;

                // 假设此时用户已经算是通过验证，或者从 Cookie 里获取真正的账号/票据
                // 这里我们暂用 ViewModel 提供的前端输入，或者如果您有解密 Token 的动作也可放置于此。
                var user = new Models.User { StudentId = ViewModel.StudentId };
